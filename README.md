...
*How it works* control — a row standing for four combos has no one combo to link to. Both
lived on the versions inside the disclosure, so a reader who wanted either had to open it
to get back what a plain row gives for free. None of that changes at four, or at eight.
So `COLLAPSE_FROM` and `groupVariants()` came out and every version became its own row.

What that looks like on a real deck is the thing the argument had not weighed:

```
                        rows   distinct combos   rows repeating a chip block
Chatterfang, unfolded    233          84                    149
Chatterfang, folded      120          84                     36
```

**149 of 233 rows repeated a block of result chips already on screen.** The versions of a
family produce identical results by construction — that is what merging them requires — so
writing them out does not add information, it adds copies. Reading down the panel meant
reading the same eight-chip block eight times before reaching anything new.

The honest counter-argument, which stands: a *collapsed* row shows its chips **N+1** times
if you open it — once on the row, then again on every version inside *All N versions*. So
the fold does not reduce repetition in the DOM, it reduces the repetition a reader scrolls
past, which is the one that costs them anything. Folding is a summary that can be expanded,
and expanding it is a choice.

Nothing else moved. The removal and the restoration were a `git revert` apart because
`byDrawnRow()`'s ordering was already the un-folded path — 99 of those 120 rows take it —
so a family reads as one block either way, and `interchangeableIn()` remains the only thing
that knows a family exists.
