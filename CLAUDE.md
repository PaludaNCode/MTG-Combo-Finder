...
- **The database is indexed, not walked.** `matchDeck()` and `standInRows()` go through
  ...
  `WeakMap` — 25ms → 5.9ms and 147ms → 11.8ms on the standing deck, identical output. **Three traps, each
  with a test**: candidates must be re-sorted into **database order** or the stable sorts downstream break
  ties differently and the page reorders (42% of combos have no `pop`, so ties are the common case); the
  index stores **one posting per occurrence**, because the old walk counted a *name* it could not find and
  counting distinct cards is a silent behaviour change; and a combo naming ≤1 card can never be reached
  through a deck card, so the 7 of them are carried separately. Nothing in the *results* says which walk
  ran → `test/scan-index.test.js` pins the count examined. README § *The database is indexed once, not
  walked on every search*.
