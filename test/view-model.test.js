...
const counted = (over) => Object.assign(
  {
    ...
    mdfc: 0, mdfcKnown: true, mapped: 1191, basicsKnown: true,
  },
  over
);
...
test('deckCountsNote: the double-faced cards are named among the spells', () => {
  assert.equal(said(View.deckCountsNote(counted({ mdfc: 3 }))),
    '98 cards · 62 spells (3 MDFCs) · 36 lands (16 basic · 20 nonbasic)');
});
...
test('deckCountsNote: no land-back list drops the aside', () => {
  assert.equal(said(View.deckCountsNote(counted({ mdfc: 3, mdfcKnown: false }))),
    '98 cards · 62 spells · 36 lands (16 basic · 20 nonbasic)');
});
