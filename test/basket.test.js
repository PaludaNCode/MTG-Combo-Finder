...
test('basket caption: the badge counts cards and the sentence says so', () => {
  ...
  const note = DeckView.basketNote(5, 33, 80);
...
test('basket caption: one card reads as one card', () => {
  const note = DeckView.basketNote(1, 33, 34);
  assert.strictEqual(note.sentence,
...
test('basket caption: without a baseline figure it claims no delta', () => {
  ...
  assert.strictEqual(DeckView.basketNote(3, null, 80).sentence,
    '3 cards that were not in the deck you started with.');
  assert.strictEqual(DeckView.basketNote(3, 33, null).sentence,
    '3 cards that were not in the deck you started with.');
});
...
test('basket caption: adding cards that changed nothing says so', () => {
  ...
  assert.strictEqual(DeckView.basketNote(2, 33, 33).sentence,
...
test('basket caption: a deck that lost combos is not phrased as a gain', () => {
  ...
  // reader's own edit.
  assert.strictEqual(DeckView.basketNote(1, 33, 28).sentence,
...
test('basket caption: nothing added is no panel', () => {
  assert.strictEqual(DeckView.basketNote(0, 33, 33), null);
});

test('basket row: the per-card figure is the one that is true now', () => {
  assert.strictEqual(DeckView.basketRowNote(18), 'in 18 combos');
  assert.strictEqual(DeckView.basketRowNote(1), 'in 1 combo');
  // A card can be added and carry nothing — Spellbook publishes combos it is in, the
  // deck holds none of the other halves. "in 0 combos" reads as a bug; this reads as
  // the situation it is.
  assert.strictEqual(DeckView.basketRowNote(0), 'in no combos yet');
});
